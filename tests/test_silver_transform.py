...
class TestDataTransformation:
    ...
    def test_manufacturer_cleaning(self):
        ...
        df = pl.DataFrame({"manufacturer": ["Honda ", " Toyota", "Ford", "  BMW  "]})
        ...
        assert result["manufacturer_original"].to_list() == [
            "Honda ",
            " Toyota",
            "Ford",
            "  BMW  ",
        ]
        assert result["manufacturer_cleaned"].to_list() == [
            "Honda",
            "Toyota",
            "Ford",
            "BMW",
    ...
    def test_gear_position_mapping(self):
        ...
        df = pl.DataFrame({"gear": ["P", "R", "N", "D", "L", "X", None]})

        result = df.with_columns(
            [
                pl.col("gear")
                .map_elements(map_gear_position, return_dtype=pl.Int64)
                .alias("gear_position")
            ]
        )
