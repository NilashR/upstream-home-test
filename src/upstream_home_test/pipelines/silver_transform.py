...
def _apply_data_transformations(df: pl.DataFrame, logger) -> pl.DataFrame:
    ...
    # Drop unused columns
    df_final = df_cleaned.drop(
        ["gearPosition", "frontLeftDoorState", "driverSeatbeltState", "wipersState"]
    )
...
def run_silver_transform(
    bronze_dir: str | None = None, output_path: str | None = None
) -> dict[str, Any]:
    ...
    try:
        ...
        error_msg = f"Parquet write error during Silver transformation: {e!s}"
        ...
        duration_ms = elapsed_ms_since(pipeline_start)
        error_msg = f"Unexpected error during Silver transformation: {e!s}"
...
def main() -> None:
    try:
        ...
        parser.add_argument(
            ...
            type=str,
            default=BRONZE_PATH,
            help="Path to Bronze parquet directory",
        )
        parser.add_argument(
            "--output-path",
            type=str,
        ...
        print(f"Silver transformation failed: {e!s}")
        sys.exit(1)
