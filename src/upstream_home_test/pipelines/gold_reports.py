...
def _run_sql_reports(
    report_names: list[str], silver_dir: str, **kwargs
) -> dict[str, Any]:
...
def run_gold_reports(
    report_names: list[str] | None = None, silver_dir: str | None = None, **kwargs
) -> dict[str, Any]:
...
def cleanup_old_parquet_files(gold_dir: str | None = None) -> None:
...
def write_reports_to_parquet(result: dict[str, Any], gold_dir: str | None = None) -> None:
    ...
    for report_name, report_result in result["results"].items():
        if report_result["status"] == "completed" and "result_data" in report_result:
            ...
            print(
                f"   ⚠️  No parquet written for {report_name} (status: {report_result['status']})"
            )
...
def main() -> None:
    try:
        ...
        parser.add_argument(
            "--reports",
            nargs="*",
            default=["fastest_vehicles_per_hour", "vin_last_state"],
            help="List of report names to run",
        )
        parser.add_argument(
            "--silver-dir",
            type=str,
        ...
        print(f"Gold reports generation failed: {e!s}")
        sys.exit(1)
